...
import { setMobileNumber, setOTPResult } from "../../actions/user-action";
...
function RegisterPhone({
  ...
  const numericOnly = e => {
    ...
    if (e.target.value === "" || regNumeric.test(e.target.value) && e.target.value.length <= 10) {
  ...
  return <>
      <TextField error={phoneNum.isError} id={phoneNum.isError ? "standard-error-helper-text" : "standard-start-adornment"} label="Enter Mobile number" className={classes.signupInputs} InputProps={{
      startAdornment: <InputAdornment position="start">+1</InputAdornment>
    }} onChange={numericOnly} value={phoneNum.value} name="phone" helperText={phoneNum.isError && `${phoneNum.errorMsg}`} />
      <Button variant="contained" className={classes.btn} id="sign-in-button" style={{
      background: "#222",
      color: "#fff"
    }} color="primary" disabled={loading} onClick={onSubmit}>
        {loading ? <CircularProgress size={24} className={classes.buttonProgress} /> : "CONTINUE"}
      </Button>
