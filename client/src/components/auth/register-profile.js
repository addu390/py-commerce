...
import { modalClose, setIsAuthenticate, setUserInfo } from "../../actions/user-action";
...
function RegisterProfile() {
  ...
  return <>
      <TextField id={errors.fName ? "standard-basic" : "standard-start-adornment"} label="Enter First Name" className={classes.signupInputs} value={values.fName} onChange={handleInputs} name="fName" error={errors.fName} helperText={errors.fName && `${errorMsg.fName}`} />
      <TextField id={errors.lName ? "standard-basic" : "standard-start-adornment"} label="Enter Last Name" className={classes.signupInputs} value={values.lName} onChange={handleInputs} name="lName" error={errors.lName} helperText={errors.lName && `${errorMsg.lName}`} />
      <FormControl className={clsx(classes.margin, classes.textField, classes.signupInputs)} error={errors.password}>
        <InputLabel htmlFor="standard-adornment-password">
          Set Password
        </InputLabel>
        <Input id={errors.password ? "standard-adornment-password" : "standard-start-adornment"} type={showPassword ? "text" : "password"} onChange={handleInputs} value={values.password} name="password" error={errors.password} endAdornment={<InputAdornment position="end">
              <IconButton aria-label="toggle password visibility" onClick={handleClickShowPassword}>
                {showPassword ? <Visibility /> : <VisibilityOff />}
              </IconButton>
            </InputAdornment>} />
        {errors.password && <FormHelperText id="standard-helper-text" error={true}>
            {errorMsg.password}
          </FormHelperText>}
      </FormControl>
      ...
      background: "#222",
      color: "#fff"
    }} color="primary" disabled={loading} onClick={submitForm}>
        {loading ? <CircularProgress size={24} className={classes.buttonProgress} /> : "Signup"}
      </Button>
    </>;
}
